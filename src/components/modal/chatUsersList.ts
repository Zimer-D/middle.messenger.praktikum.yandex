...
export default class ChatUsersList extends Block {
 ...
 Client
            .post(`${URLS.API_URL}/user/search`, {data: JSON.stringify(data)})
            .then((res: any) => { this.setProps({searchedUsers: res}) })
            .catch((e) => {
                console.log(e);
            })
    }
    ...
    render() {
        ...
        const temp = `<div class="modal-wrapper ${isOpenedClass}">
                ...
                )).join(' ') : 'Нет приглашенных пользоватей'}
                                     <div>
                                     <div>
                                        ...
                                        ${!!this.props.searchedUsers&& !!this.props.searchedUsers.length?
                                            this.props.searchedUsers.map(q=>(
